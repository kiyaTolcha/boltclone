async function queryOsvDb(packageName, version, ecosystem = 'npm') {
  ...
  try {
    ...
    return vulns.map((v) => ({
      id: v.id, // e.g. GHSA-xxxx or CVE-2023-xxxx
    ...
    console.warn('OSV.dev query error:', err);
    return [];
  }
}

async function lookupSoftwareVulns(serverHeader) {
  ...
  for (const match of matches) {
    ...
    if (name && version) {
      // Query PyPI, npm, or generic ecosystem
...
if (typeof self !== 'undefined') {
  self.queryOsvDb = queryOsvDb;
  self.lookupSoftwareVulns = lookupSoftwareVulns;
} else if (typeof window !== 'undefined') {
  window.queryOsvDb = queryOsvDb;
  window.lookupSoftwareVulns = lookupSoftwareVulns;
}
if (typeof globalThis !== 'undefined') {
  globalThis.queryOsvDb = queryOsvDb;
  globalThis.lookupSoftwareVulns = lookupSoftwareVulns;
}
